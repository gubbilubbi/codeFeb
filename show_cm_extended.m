...
%         end
        ...
        if foo_plotDS==true
            ...
            if ~isempty(pp.FctForColSph) % plot function on vertex instead of degree
                ...
                if ~isempty(pp.ColAxis), caxis(pp.ColAxis);%here
                end
            else
            ...
            % This makes it so that the colors for each row is
            % between it's min and max and not the global min and max
            % which is CA which is in pp.ColAxis
...
invCMs={'autumn','bone','hot','copper','pink'};
