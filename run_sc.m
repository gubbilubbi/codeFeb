...
% Calculate SDI and save it?
    % They use the l2-norm + some thresholding for every subject
    % Check their code GSP_HandsOn for this :D
    %
...
data=load_data('/archive/21098_pd_dimensionality/MEG/',44,startInd:endInd,idsHC);% time x regions x subjects
%data=load_data('scilife/MEG/',44,startInd:endInd,idsHC);% time x regions x subjects
...
% Calculate FC matrix for each subject and mean FC for an entire group
...
X_hat=zeros(nROI,dataLength,nSubjs);
...
%saveFolder="./scilife/testData/"+folderName;
...
% Random noise used for testing
%PSD=PSD+rand(nROI,nSubjs);
...
for g=1:nGroups
    ...
    idx(:,g)=max(PSD(:,indsPDHC{g}(1))')>0 & min(PSD(:,indsPDHC{g}(1))')>0 & mean(PSD(:,indsPDHC{g}(1))')>0;
    
    % Mean power spectrum density over each group
    mPSD(:,g)=PSD(:,indsPDHC{g}(1));
    ...
    % For cutoff lower than 50% of total AUC do -2 with this implementation
    psdNN(g)=n(g)-2;
    ...
    % Save the PSD plots
    %if(boolSavePlot)
    saveas(psdFig,saveMiscFolder+"/psd"+plotTitles(g)+".png")
    %end
end
...
psdFolder=saveFolder+"/psd_"+strCutoffs+"/";
...
Xc=zeros(dataLength,nROI,nSubjs);
Xd=zeros(dataLength,nROI,nSubjs);
...
for low=3:18 %4:22
    ...
    for up=27:42 %23:41
        ...
        cutoffFolder=saveFolder+"/cutoff_"+strCutoffs+"/";
        if ~exist(cutoffFolder, 'dir')
            mkdir(cutoffFolder);
        end
        ...
        Xc=zeros(dataLength,nROI,nSubjs);
        Xm=zeros(dataLength,nROI,nSubjs);
        Xd=zeros(dataLength,nROI,nSubjs);
        for s=1:nSubjs
            % reconstruction of coupled signal, low, medium and high freqs.
            Xc(:,:,s)=(Ulow(:,:,s)*X_hat(:,:,s)).';
