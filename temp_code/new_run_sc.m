...
laplacianType="normLaplacian"; % Laplacian or normLaplacian
networkType="Mean FC"; % Indvidual FC, Session FC, Mean FC, SC
...
plotLegends=["PD OFF","PD ON","HC ses1","HC ses2"];
...
A=zeros(nROI,nROI,nScans);
SC=zeros(nROI,nROI,nScans);
switch networkType
    case 'Indvidual FC'
        A=abs(FC);

    case "Session FC" % session-wise network (meanFC for PD1, PD2, HC1, HC2)
        A=abs(meanFC);

    case 'Mean FC'
        for j=[1,3]
            inds=[indsPDHC{j},indsPDHC{j+1}];
            meanConditionFC=mean(meanFC(:,:,[indsPDHC{j},indsPDHC{j+1}]),3);
            for i=inds
                A(:,:,i)=abs(meanConditionFC);
            end
        end
       
    case "SC" %
        % TODO Add code for loading SC here
        A=SC;
...
folderName=normMethod+"_"+networkType+"="+laplacianType+...
...
for g=1:nGroups
    ...
    avgPSD(:,g)=mean(PSD(:,indsPDHC{g}),2);
    stdPSD(:,g)=std(PSD(:,indsPDHC{g}),0,2); 
    upper1(:,g)=avgPSD(:,g)+stdPSD(:,g);
    lower1(:,g)=avgPSD(:,g)-stdPSD(:,g);
    ...
    % If negative value in first Lambda we can avoid it by
    %psdInds=2:nROI;
    % Otherwise just comment the above row and remove the below comment
    psdInds=1:nROI;

    psdFig=figure;
    patch([LambdaL(psdInds,indsPDHC{g}(1))', fliplr(LambdaL(psdInds,indsPDHC{g}(1))')], ...
       [lower1(psdInds,g)' fliplr(upper1(psdInds,g)')], [0.8 0.8 0.8]);
    hold on;
    % One lambda per group
    plot(LambdaL(psdInds,indsPDHC{g}(1)),mPSD(psdInds,g));
    %xlim([0.05 2]);ylim([1 50]);
    title("Spectral Energy Density "+plotLegends(g));xlabel('Harmonic Frequency');ylabel('Energy')
    %xline(LambdaL(psdNN(g))) % Plot cutoff
    set(gca, 'XScale', 'log', 'YScale','log')
    % Save the PSD plots
    %if(boolSavePlot)
    saveas(psdFig,saveMiscFolder+"/psd_group_"+plotLegends(g)+".png")
    %end
end
...
save(psdFolder+"/avgPSD.mat",'avgPSD')
save(psdFolder+"stdPSD.mat","stdPSD")
...
% 
%     % disp("Energy split "+plotLegends(g))
%     % disp(splitLow)
%     % disp(splitMedium)
%     % disp(splitHigh)
...
%     Ulow(:,1:psdNN(g,1),indsPDHC{g})=U(:,1:psdNN(g,1),indsPDHC{g}); 
%     %medium spatial frequencies
%     Umedium(:,psdNN(g,1)+1:psdNN(g,2)-1,indsPDHC{g})=U(:,psdNN(g,1)+1:psdNN(g,2)-1,indsPDHC{g});
%     %high spatial frequencies
%     Uhigh(:,psdNN(g,2):end,indsPDHC{g})=U(:,psdNN(g,2):end,indsPDHC{g}); 
% end
% 
% %%% reconstruct functional signals containing only low / high frequencies
% % This both filters and inverts the data back to time domain 
...
for s=1:nScans
    ...
    for lowC=1:nROI
        ...
        % If the absolute difference is smaller then update the 
        % best cutoff values for the most even AUC split (50/50)
        if(totDiff < minDiff)
            psdNN_lh(s)=lowC;
            minDiff=totDiff;
        end
...
psdSingleLHFolder=saveFolder+"/psd_single_lh/";
if ~exist(psdSingleLHFolder, 'dir')
       mkdir(psdSingleLHFolder);
end

% psdSingleLMHFolder=saveFolder+"/psd_single_lmh/";
% if ~exist(psdSingleLMHFolder, 'dir')
%        mkdir(psdSingleLMHFolder);
% end

save(psdSingleLHFolder+"/PSD.mat",'PSD');
%save(psdSingleLMHFolder+"/PSD.mat",'PSD');
save(psdSingleLHFolder+"/psdNN_lh.mat",'psdNN_lh');
%save(psdSingleLMHFolder+"/psdNN_lmh.mat",'psdNN_lmh');


single_psd_energy_split(U,X_hat,nScans,psdNN_lh,psdSingleLHFolder)
%single_psd_energy_split(U,X_hat,nScans,psdNN_lmh,psdSingleLMHFolder)

psdFolder=psdSingleLHFolder;
%% Create SC-informed graph signal surrogates by randomization of real harmonics Fourier coefficients

% Inspiration from Preti and Van De Ville
% https://doi.org/10.1038/s41467-019-12765-7
% https://github.com/gpreti/GSP_StructuralDecouplingIndex

nSurr=19; % 19 surrogates for 1/(1+19)=0.05 significance level alpha
